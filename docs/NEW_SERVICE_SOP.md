...
3.  **规则定义**: `src/entity/relationship/newObjectRules.json`
    ...
    - **`codebase` 与 `codeList` 详解**:
      - **`codebase`**: 这是规则的"蓝图"，用分号分隔不同的元素。它可以使用 `|` (或) 和 `&` (与) 来定义组合逻辑。
      - **`codeList`**: 这是生成器实际使用的、确定的元素组合列表。**您必须手动将 `codebase` 中的所有逻辑可能性展开，并写入 `codeList`**。
      - **`|` (或) 规则**: `|` 仅作用于它所在的分号分割块中。例如 `A|B|C` 意味着这一"格"可以是 `A` 或 `B` 或 `C`。
      - **示例**: 如果 `codebase` 是 `01;04|07|08;03;02`，那么正确的 `codeList` 应该是 `["01;04;03;02", "01;07;03;02", "01;08;03;02"]`。`codecount` 应为4。
      - **注意**: `&` 逻辑（如 `(09&11)`）应被视为一个不可分割的原子块。
