...
class StaffingUpdateService(BaseGenerationService):
    """
    ...
    def _get_rule_by_id(self, rule_id):
        ...
        if not hasattr(self, '_rules_cache'):
            ...
            try:
                ...
                rules_file_path = os.path.join(ENTITY_DIR, 'relationship', 'updateStaffRules.json')
    ...
    def post_process_data(self, data, answer_elements):
        ...
        for item in data:
            ...
            if any(keyword in question_text for keyword in ["添加", "安排"]):
                item['answer']['operation'] = "添加"
            # 检查问题中是否包含删除/撤销/移除相关词汇
            elif any(keyword in question_text for keyword in ["删除", "撤销", "移除"]):
                item['answer']['operation'] = "删除"
            else:
                item['answer']['operation'] = "查询"  # 默认值
    ...
    def generate_update_staffing_data(self, business_object: str = BUSINESS_OBJECT,
...
# 获取服务实例的便捷函数
def get_staffing_update_service():
    return StaffingUpdateService.get_instance()


# 便捷方法，使用变种生成服务创建
def generate_update_staffing_data(business_object: str = StaffingUpdateService.BUSINESS_OBJECT,
    ...
    staffing_update_service = get_staffing_update_service()

    # 调用生成方法，传递变种服务实例
    return staffing_update_service.generate_update_staffing_data(
        business_object,
        total_samples,
        variations_per_rule,
...
if __name__ == "__main__":
    try:
        ...
        staffing_data = generate_update_staffing_data(total_samples=10, variations_per_rule=2)
...
def clear_rules_cache():
    ...
    # 实际的缓存会在实例重新创建时自动清理
    import logging
    logging.info("StaffingUpdateService: 规则缓存已标记为需要清理")
