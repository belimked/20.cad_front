...
class SearchPOService(BaseGenerationService):
    ...
    def post_process_data(self, data, answer_elements):
        ...
        for item in data:
            ...
            if 'timeRange' in item['question'] and (
                    'orderDate' in item['question'] or 'orderAction' in item['question'] or 'auditDate' in item['question'] or 'auditAction' in item['question']):
                # 使用工具函数标准化工号
...
def generate_search_order_data(business_object: str = SearchPOService.BUSINESS_OBJECT,
    ...
    # 调用生成方法
    return search_order_service.generate_data(
        total_samples=total_samples,
        variations_per_rule=variations_per_rule,
        rule_ids=ruleids
    )
...
def clear_rules_cache():
    ...
    # 实际的缓存会在实例重新创建时自动清理
    import logging
    logging.info("SearchPOService: 规则缓存已标记为需要清理")
