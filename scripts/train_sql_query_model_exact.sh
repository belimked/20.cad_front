...
OUTPUT_DIR="${OUTPUT_BASE_DIR}/sql_query_training_exact_${TIMESTAMP}"
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/sql_training_exact_${TIMESTAMP}.log"
PID_FILE="${LOG_DIR}/sql_training_exact_${TIMESTAMP}.pid"
...
show_config() {
    ...
    echo "模型类型: qwen2_5 (精确匹配)" | tee -a "$LOG_FILE"
    ...
    echo "LoRA配置: rank=8, alpha=32 (基于成功配置)" | tee -a "$LOG_FILE"
    echo "===================" | tee -a "$LOG_FILE"
}

# 启动训练
start_training() {
    print_info "🚀 开始后台训练 SQL 查询生成模型 (精确匹配版)..."
    ...
    # 记录训练命令
    echo "精确匹配版双卡分布式训练命令:" | tee -a "$LOG_FILE"
...
main() {
    ...
    print_info "=== SQL查询生成模型精确匹配版后台训练脚本 ==="
    print_info "启动时间: $(date)"
    print_info "基于成功配置: model_type=qwen2_5, batch_size=2, grad_accum=8"
    ...
    print_success "精确匹配版脚本执行完成！训练正在后台进行..."
