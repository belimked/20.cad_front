...
"""
SummaryCargoService 使用示例

这个示例展示了如何使用新创建的 SummaryCargoService 来生成货单统计相关的训练数据。
"""
...
from src.service.summary_cargo_service import generate_summary_cargo_data, get_summary_cargo_service


def main():
    """
    主函数，演示 SummaryCargoService 的各种用法
    """
    print("=" * 60)
    print("SummaryCargoService 使用示例")
    print("=" * 60)
    ...
    print("-" * 40)
    try:
        test_data = generate_summary_cargo_data(
            total_samples=5,
    ...
    print("-" * 40)
    try:
        specific_data = generate_summary_cargo_data(
            total_samples=3,
            variations_per_rule=1,
            ruleids="1"
        )
    ...
    print("-" * 40)
    try:
        service = get_summary_cargo_service()
    ...
    try:
        # 生成一些数据
        export_data = generate_summary_cargo_data(
            total_samples=10,
            variations_per_rule=1
        )
        
        # 保存到JSON文件
        output_file = "summary_cargo_samples.json"
